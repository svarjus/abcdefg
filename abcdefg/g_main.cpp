...
void g::G_PrepareHooks()
{
	...
	OutskirtsCodeGenerator__get_Code				= (uintptr_t)(GameAssembly + 4409984);									//OutskirtsCodeGenerator$$get_Code
	...
	PlayerController_Fire_Delay						= (GameAssembly + 4430592 + 0x137);	//PlayerController$$Fire + 0x137
	PlayerController_Fire_Recoil					= (GameAssembly + 4430592 + 0x23D);  //PlayerController$$Fire + 0x23D			--> StressReceiver.InduceStress
	PlayerController_Fire_Effect					= (GameAssembly + 4429088 + 279);   // PLayerController$$FireLocal + 279		--> Item.Fire

	SteamDLCManager__HasDLC_h						= (bool(*)(int32_t dlc))(g::GameAssembly + 2963200); //SteamDLCManager$$HasDLC

	OutskirtsKeyPad_Press_f							= (void(*)(void* keypad, int num, const MethodInfo * method))(GameAssembly + 4411248); //OutskirtsKeypad$$Press
